...
SETTINGS = mongodb["reaction_settings"]  # reaction ON/OFF switch
...
# Filter config reactions safely
...
async def load_reaction_state():
    ...
    print(f"[Reaction Switch] Loaded state: {REACTION_ENABLED}")
...
async def toggle_reaction(client, message: Message):
    ...
    if not ok:
        return await message.reply_text(
            f"⚠️ Only admins/sudo users can do that.\nDebug: {debug or 'unknown'}"
        )

    if len(message.command) < 2:
        return await message.reply_text(
            "**Usage:**\n"
            "`/react on` — enable reactions\n"
            "`/react off` — disable reactions\n"
            "`/react status` — show current mode"
        )
    ...
    if mode == "on":
        ...
        return await message.reply_text("✅ **Reactions enabled.**")

    elif mode == "off":
        REACTION_ENABLED = False
        await SETTINGS.update_one({"_id": "switch"}, {"$set": {"enabled": False}}, upsert=True)
        return await message.reply_text("🛑 **Reactions disabled.**")

    elif mode == "status":
        return await message.reply_text(
            f"🔍 **Reactions are:** "
            + ("`ON` ✅" if REACTION_ENABLED else "`OFF` ❌")
        )

    else:
        return await message.reply_text("❌ Invalid option. Use `/react on`, `/react off`, or `/react status`.")
...
async def delete_reaction_name(client, message: Message):
    ...
    if removed:
        await message.reply_text(f"🗑 Removed `{raw}` from reaction triggers.")
    else:
        await message.reply_text(f"❌ `{raw}` not found in reaction triggers.")
...
async def list_reactions(client, message: Message):
    if not custom_mentions:
        return await message.reply_text("ℹ️ No reaction triggers saved.")
...
async def clear_reactions(client, message: Message):
    ...
    if not ok:
        return await message.reply_text(
            f"⚠️ Only admins or sudo users can clear reaction triggers.\n\nDebug info:\n{debug or 'unknown'}"
        )

    await COLLECTION.delete_many({})
    custom_mentions.clear()
    await message.reply_text("🧹 All reaction triggers cleared.")
